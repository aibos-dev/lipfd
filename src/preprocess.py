...
# def get_spectrogram(audio_file, device="cuda"):
#     """Generate Mel spectrogram on GPU."""
#     data, sr = librosa.load(audio_file)
#     data = torch.tensor(data, device=device)
#     mel_filter = librosa.filters.mel(sr=sr, n_fft=2048, n_mels=128)  # 修改此行
#     mel_filter = torch.tensor(mel_filter, device=device)
#     stft = torch.stft(data, n_fft=2048, hop_length=512, return_complex=True)
#     spectrogram = (stft.real**2 + stft.imag**2).sqrt()
#     mel = mel_filter @ spectrogram
#     mel_db = 10 * torch.log10(torch.clamp(mel, min=1e-10))
#     mel_db = mel_db.cpu().numpy()
#     plt.imsave("./temp/mel.png", mel_db)
